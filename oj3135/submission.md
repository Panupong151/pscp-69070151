...
```text
inputเป็นจำนวนเต็มบรรทัดเดียว มี N K T เงื่อนไขคือ มี ขโมยแฝงตัวอยู่หนึ่งคน นั่นคือคนหมายเลขT
การส่งของขวัญนี้จะหยุดลงเมื่อเกิดเหตุการณ์ใดเหตุการณ์หนึ่ง คือ ของขวัญวนกลับมาถึงมือคนที่ 1 อีกครั้ง หรือ ของขวัญถูกส่งตรงเข้ามือขโมย ซึ่งขโมยจะคว้าของขวัญแล้ววิ่งหนีไปทันที
outputเป็นจำนวนคนที่ได้พิจารณาของขวัญ
```
...
```text
Step 1:รับค่าinput และ กำหนดค่าเริ่มต้น
Step 2:เช็คว่าถ้า T == 1 (คนที่ 1 เป็นขโมยตั้งแต่แรก) ให้แสดงผลลัพธ์ 1 แล้วจบโปรแกรมทันที
Step 3:ใช้loopในการพิจารณาการส่งของขวัญคำนวณตำแหน่งถัดไป: ใช้สูตร next_person = (current - 1 + K) % N + 1
ตรวจเงื่อนไขจบที่ถ้า next_person == 1 ให้ break ออกจากลูปทันที
นับจำนวนคน บวกเพิ่ม count += 1
ตรวจเงื่อนไขจบที่ ถ้า next_person == T ให้ break ออกจากลูปทันที
อัปเดตตำแหน่ง เปลี่ยน current = next_person เพื่อเตรียมส่งต่อในรอบถัดไป
Step 4:แสดงผลOutput
```
...
```text
ไม่ต่างจากแผนแรกเพราะมันสามารถทำงานได้
```
...
```text
ทดสอบว่าระบบไม่นับขโมยหากของขวัญข้ามหัวไป
```
