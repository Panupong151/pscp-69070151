...
```text
Inputคือพื้นที่ที่น้ำหมึกแผ่ขยายไปต่อหนึ่งวินาที จำนวนคนที่ร้องขอ และพิกัดบ้านของจำนวนคน
Outputคือระยะเวลาที่บ้านหลังนั้นจะโดนท้วม
Constraintsคือ S (พื้นที่หมึกขยายต่อวินาที): 1 <= S <= 100,000
N (จำนวนคน/บ้าน): 1 <= N <= 100
X, Y (พิกัดบ้านแต่ละหลัง): 0 <= X, Y <= 10,000
ค่า pi ที่โจทย์กำหนดให้ใช้: 3.1416
```
...
```text
Step 1:รับค่าการขยายพื้นที่และคำร้องเข้ามา
Step 2:นำไปคำนวนในสูตร(ก่อนใช้aiไม่รู้ว่าจะใช้สูตรไหนดี)
Step 3:แสดงผล
```
...
```text
เริ่มจากการรับค่าอัตราการขยายพื้นที่และจำนวนคำร้องจากนั้นรับพิกัด(X, Y) ของบ้านแต่ละหลังเข้ามาคำนวณพื้นที่วงกลมที่น้ำหมึกต้องแผ่ไปถึงด้วยสูตร $A = 3.1416 \times (X^2 + Y^2) แล้วนำพื้นที่ที่ได้มาหารด้วยเพื่อหาเวลาที่ใช้ จากนั้นนำผลลัพธ์มาปัดเศษทศนิยมขึ้นเป็นจำนวนเต็มด้วยฟังก์ชัน math.ceil() ตามเงื่อนไขโจทย์ บันทึกผลลัพธ์แล้ววนลูปพิมพ์แสดงผล
```
...
```text
ทดสอบกรณีที่มีบ้านอยู่ตรงกลางหมึก
```
